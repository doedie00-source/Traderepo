...
function DupeTab:OnAddAllCrates(cratesList, inventoryCrates)
    ...
    task.spawn(function()
        ...
        for _, crate in ipairs(cratesList) do
            ...
            -- เช็คว่ายังไม่มีในตัว และยังไม่ได้เลือกไว้
            if amountInInv == nil and not self.StateManager.selectedCrates[crate.DisplayName] then
                
                -- ✨ [จุดที่แก้] บันทึกลง State ว่าเลือก 1000 ชิ้น (เพื่อให้ UI โชว์ x1000)
